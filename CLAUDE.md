...
- **Security First (DEC-013):** ninguna funcionalidad nueva accede a la API sin autenticación (única excepción: `GET /salud`). Supabase Auth mediada por la API — la PWA nunca habla directo con Supabase Auth ni almacena credenciales (solo access token en memoria y refresh token local con rotación). Autorización siempre en la API vía RBAC propio. Ninguna regla de negocio migra fuera de `packages/dominio`.
