...
**Dominio congelado; API aprobada.** `@cuadreapp/dominio` con 77 pruebas y cobertura 100%. `apps/api` con `POST /api/v1/cargas` aprobada (13 pruebas HTTP) más observabilidad por defecto (DEC-012). En curso: integración PWA (captura → cola offline → sincronización). Pendientes de fases posteriores: autenticación, dashboard, entregas. La PWA usa `@cuadreapp/dominio` para feedback inmediato pero nunca reescribe reglas; el veredicto del servidor es la autoridad.
