...
**Dashboard en implementación** (`apps/dashboard`, DEC-015): frontend con datos simulados detrás de adaptadores (`docs/DASHBOARD_ARQUITECTURA.md`); sin API, sin autenticación, alcance cerrado. **Etapa H (Hardening) implementada — 154 pruebas en verde.** Gate local completo: `pnpm verificar` (lint + formato + fronteras DEC-007 + typecheck + pruebas). E2E contra infraestructura real: `pnpm --filter @cuadreapp/api e2e` con las variables `E2E_*` (docs/OPERACIONES.md). CI en `.github/workflows/ci.yml`. Operación, despliegue y respaldos: `docs/OPERACIONES.md`.
