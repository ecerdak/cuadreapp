...
**Etapa P.3 (Accesos al Dashboard + UX Hardening P0) DESPLEGADA** (12-ago-2026, DEC-021): el alta de usuarios del Dashboard vive en la consola (ficha del cliente → Dashboard → Accesos, sin SQL; en producción desde el 10-ago) y el **Hardening P0** corre en producción desde el 12-ago (`b9c919f`, API/Admin/Dashboard por GitHub; migración `20260812090000` aplicada y verificada ANTES del push, historial de migraciones reparado, Supabase Auth con Redirect URL y `URL_RESTABLECER_PASSWORD` configuradas): contraseña temporal de UN ingreso con ciclo completo (cambio obligatorio en el primer login, cambio voluntario, «olvidé mi contraseña» y `/restablecer`), entrega de credenciales copiable a WhatsApp sin pérdida silenciosa, estados de acceso honestos (revocado ≠ expirado; ninguna pantalla sin salida), `mensajeDeError()` conectado (frase humana + `request_id` copiable), bienvenida del cliente nuevo con transición automática a la primera carga (`tiene_cargas` en `/tablero/hoy`), Excel por Perfil Operativo (sin N+1 en inventario; medidor sin regresión) y confirmaciones en desactivar/regenerar. Evidencia: `docs/capturas/p0-ux-hardening/` (regenerable con `node scripts/capturar-p0.mjs`). Verificación automática en verde (salud 20/20, contratos de auth, bundles servidos con el P0); **pendiente el smoke humano** (primer acceso real, ciclo de contraseña, revocación, aislamiento) y reconciliar la trazabilidad Git de la PWA (watch path en Railway; la PWA sirve el artefacto CLI del 10-ago con los fixes del operador verificados en el bundle — detalle en CHANGELOG y OPERACIONES).
