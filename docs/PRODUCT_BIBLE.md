...
**Versión:** 1.7 — 31 de julio de 2026
**Estado del proyecto:** Dominio congelado. `apps/api` con `POST /api/v1/cargas` **aprobada**. Observabilidad por defecto registrada (DEC-012). Integración con la PWA en curso (captura → cola offline → sincronización). Verificación de Etapa 0 sigue pendiente de un entorno con Postgres.
...
| **1** | Flujo de conductor completo, offline, con R1–R12 validadas contra la API propia (Railway) | **En progreso** — dominio **congelado**; API `POST /api/v1/cargas` **completada y aprobada** con observabilidad por defecto ([DEC-012](#decisiones)); integración PWA en curso (captura → cola offline → sincronización) | 10 cargas registradas en modo avión y sincronizadas al recuperar señal |
