# OP-IOS-CI · Operadores iPhone — perfil Carga sobre Inventario
...
> Layout: [`../05_Layouts/OP-IOS-CI.md`](../05_Layouts/OP-IOS-CI.md) · Checklist: [`../06_Checklists/OP-IOS-CI.md`](../06_Checklists/OP-IOS-CI.md) · Troubleshooting: [`../07_Troubleshooting/OP-IOS-CI.md`](../07_Troubleshooting/OP-IOS-CI.md) · Video: [`../08_Storyboards/OP-IOS-CI.md`](../08_Storyboards/OP-IOS-CI.md)

**Relación con OP-IOS-MD:** comparte las páginas 1–7 y 12–14 (instalación, enrolamiento, inicio, equipo, operador, cierre y reglas). Cambian **las páginas 8 a 11**, que son la captura propia de este perfil. Si se corrige algo en las páginas compartidas, hay que corregirlo en los dos.
...
|                        |                                                                                                                                          |
| ---------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| **Audiencia**          | Operadores que atienden carrotanques en sitio. Teléfono iPhone.                                                                          |
...
| **Prerrequisitos**     | iPhone con Safari (obligatorio) · código de enrolamiento del supervisor · código de operador y PIN · saber leer la placa del carrotanque |
| **Perfil operativo**   | Carga sobre Inventario — no hay medidor de tanda ni totalizador                                                                          |
| **Páginas estimadas**  | 13                                                                                                                                       |

**Diferencias frente a la versión Android:** la instalación se hace desde Compartir en Safari (no hay botón automático), borrar los datos de Safari borra la app, y «Almacenamiento protegido» puede quedar en «NO» con más frecuencia. Las pantallas de captura son idénticas.
...
- **Título:** Registrar una carga con CuadreApp · **Subtítulo:** Guía del operador · iPhone
- **Pantalla:** `ios-01_splash.png` · **Callouts:** ninguno
- **Texto:** «Dos fotos y dos números. El total lo calcula la app.»

### Página 2 · Instalar la app _(una sola vez)_

- **Título:** Primero: instala la app _(tiene que ser en Safari)_ · **Pantalla:** `ios-15_instalar.png`
- **Callouts:** `C-INS-02` (Alta), `C-INS-01` (Alta), `C-INS-03` (Alta)
- **Texto:** abrir **en Safari** → Compartir (el cuadrado con la flecha) → «Añadir a pantalla de inicio» → «Añadir» → abrir siempre desde el ícono.
- **Advertencia propia de iPhone:** borrar Safari o los «datos de sitios web» borra la app y las cargas sin subir.

### Página 3 · Enrolar el teléfono _(una sola vez, con señal)_

- **Título:** Conecta el teléfono a tu operación · **Pantalla:** `ios-02_enrolar.png`
- **Callouts:** `C-ENR-01` (Alta), `C-ENR-02` (Media), `C-PER-01` (Alta)

### Página 4 · La pantalla de inicio

- **Título:** Tu pantalla de todos los días · **Pantalla:** `ios-03_inicio.png`
...
- **Pantallas:** `ios-06_operador-codigo.png` y `ios-07_operador-pin.png`
...
- **Pantalla:** `ios-11_llegada.png` (grande)
...
- **Título:** Paso 4 · Despacha el combustible · **Pantalla:** `ios-09_cargando.png`
...
- **Pantalla:** `ios-12_despacho.png` (grande)
...
- **Pantalla:** `ios-12_despacho.png` recortada a la tarjeta de tres cifras
...
- **Título:** Listo. Ya quedó registrada. · **Pantalla:** `ios-13_listo.png`
...
- **Título:** Si algo sale distinto · **Pantalla:** `ios-14_diagnostico.png` (pequeña, lateral)
...
**12 capturas.** Nueve compartidas con OP-IOS-MD (se producen una sola vez) y **dos exclusivas de este perfil**.

| #   | Archivo                      | Pantalla | Compartida | Estado a capturar                                                                                              |
| --- | ---------------------------- | -------- | ---------- | -------------------------------------------------------------------------------------------------------------- |
| 1   | `ios-01_splash.png`          | PWA-01   | sí         | Splash completa                                                                                                |
| 2   | `ios-15_instalar.png`        | PWA-15   | sí         | Hoja Compartir de Safari                                                                                       |
| 3   | `ios-02_enrolar.png`         | PWA-02   | sí         | Código escrito, botón amarillo                                                                                 |
| 4   | `ios-03_inicio.png`          | PWA-03   | sí         | 2–3 cargas, chip sincronizado                                                                                  |
| 5   | `ios-04_equipo-lista.png`    | PWA-04   | sí         | Lista filtrada                                                                                                 |
| 6   | `ios-05_equipo-confirma.png` | PWA-05   | sí         | Tarjeta verde                                                                                                  |
| 7   | `ios-06_operador-codigo.png` | PWA-06   | sí         | Campo vacío                                                                                                    |
| 8   | `ios-07_operador-pin.png`    | PWA-07   | sí         | Dos puntos llenos                                                                                              |
| 9   | **`ios-11_llegada.png`**     | PWA-11   | **no**     | Foto tomada y «150,0» escrito                                                                                  |
| 10  | `ios-09_cargando.png`        | PWA-09   | sí         | Cronómetro en 01:20                                                                                            |
| 11  | **`ios-12_despacho.png`**    | PWA-12   | **no**     | Foto tomada, «600,0» escrito y la tarjeta mostrando 750,0                                                      |
| 12  | `ios-13_listo.png`           | PWA-13   | parcial    | **Variante:** la tarjeta debe mostrar Llegó con / Despachado / Total al salir → `ios-13b_listo-inventario.png` |
| 13  | `ios-14_diagnostico.png`     | PWA-14   | sí         | Valores normales                                                                                               |

**Nota de producción:** `ios-13_listo.png` NO se puede compartir entre perfiles: la tarjeta de resumen es distinta. Se necesitan las dos variantes.
...
**18 callouts.** Comparte con OP-IOS-MD los de instalación, enrolamiento, inicio, equipo, operador, cargando, listo y diagnóstico. Los propios:
...
Matriz en [`../07_Troubleshooting/OP-IOS-CI.md`](../07_Troubleshooting/OP-IOS-CI.md).
